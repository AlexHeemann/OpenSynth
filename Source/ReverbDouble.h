...
#ifndef JUCE_REVERBDOUBLE_H_INCLUDED
#define JUCE_REVERBDOUBLE_H_INCLUDED

// Adapted version of the JUCE reverb class to work with doubles as well as floats.
...
public:
    ...
    void processStereo (FloatType* const left, FloatType* const right, const int numSamples) noexcept
    {
        jassert (left != nullptr && right != nullptr);
        
        for (int i = 0; i < numSamples; ++i)
        {
            const FloatType input = (left[i] + right[i]) * gain;
            FloatType outL = 0, outR = 0;
    ...
    template <typename FloatType>
    void processMono (FloatType* const samples, const int numSamples) noexcept
    {
        jassert (samples != nullptr);
        
        for (int i = 0; i < numSamples; ++i)
        {
            const FloatType input = samples[i] * gain;
            FloatType output = 0;
...
private:
    ...
    {
    public:
        CombFilter() noexcept   : bufferSize (0), bufferIndex (0), lastFloat (0), lastDouble (0)  {}
        ...
        void setSize (const int size)
        {
            ...
            {
                bufferIndex = 0;
                bufferFloat.malloc ((size_t) size);
                bufferDouble.malloc ((size_t) size);
                bufferSize = size;
            }
        ...
        void clear() noexcept
        {
            lastFloat = 0;
            lastDouble = 0;
            bufferFloat.clear ((size_t) bufferSize);
            bufferDouble.clear ((size_t) bufferSize);
        }
        
        float process (const float input, const float damp, const float feedbackLevel) noexcept
        {
            return processWithBuffer(input, damp, feedbackLevel, bufferFloat, lastFloat);
        }
        
        double process (const double input, const float damp, const float feedbackLevel) noexcept
        {
            return processWithBuffer(input, damp, feedbackLevel, bufferDouble, lastDouble);
        }
        
    private:
        HeapBlock<float> bufferFloat;
        HeapBlock<double> bufferDouble;
        ...
        template <typename FloatType>
        FloatType processWithBuffer (const FloatType input, const float damp, const float feedbackLevel, HeapBlock<FloatType>& buffer, FloatType& last) noexcept
        {
            const FloatType output = buffer[bufferIndex];
            last = (output * (1.0f - damp)) + (last * damp);
            JUCE_UNDENORMALISE (last);
            
            FloatType temp = input + (last * feedbackLevel);
    ...
    public:
        ...
        {
            return processWithBuffer(input, bufferFloat);
        }
        
        double process (const double input) noexcept
        {
            return processWithBuffer(input, bufferDouble);
        }
    ...
    private:
        ...
        template <typename FloatType>
        FloatType processWithBuffer (const FloatType input, HeapBlock<FloatType>& buffer) noexcept
        {
            const FloatType bufferedValue = buffer [bufferIndex];
            FloatType temp = input + (bufferedValue * 0.5f);
            JUCE_UNDENORMALISE (temp);
            buffer [bufferIndex] = temp;
            bufferIndex = (bufferIndex + 1) % bufferSize;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ReverbDouble)
};
